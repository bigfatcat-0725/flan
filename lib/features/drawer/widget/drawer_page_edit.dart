...
class DrawerPageEdit extends HookConsumerWidget {
  final int toSeq;
  final Pages page;
  const DrawerPageEdit({
    Key? key,
    required this.toSeq,
    required this.page,
  }) : super(key: key);
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    useEffect(() {
      if (context.mounted) {
        privateCheck.value = page.private == 0 ? true : false;
        titleTextController.text = page.title.toString();
        questionTextController.text = page.content.toString();
        if (!addOne.value) {
          if (page.tag.toString() != '') {
            var defaultTag = page.tag.toString().split(',');
                  ...
                  children: [
                    GestureDetector(
                      onTap: () async {
                        ...
                        if (image != null) {
                          var defaultMax = 3 -
                              (page.photo.toString().split(',')[0] == ''
                                  ? 0
                                  : page.photo.toString().split(',').length);
